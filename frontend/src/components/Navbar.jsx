...
const Navbar = () => {
  ...
  return (
    ...
    <div className="hidden md:flex items-center justify-center space-x-2 md:space-x-4">
      {user? <h3 className="text-[#363b56]"><Link to="/write">Write</Link></h3> :<h3 className="text-[#363b56]"><Link to="/login">Login</Link></h3>}
      {user? <div onClick={showMenu}>
        <p className="text-[#363b56] cursor-pointer relative"><FaBars/></p>
        {menu && <Menu/>}
      </div>:<h3 className="text-[#363b56]"><Link to="/register">Register</Link></h3>}
    </div>
